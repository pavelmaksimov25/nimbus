.PHONY: help install migrate migrate-up migrate-down migrate-create migrate-status run build clean docker-build docker-up docker-down docker-logs test lint docs security security-gosec security-govulncheck security-staticcheck
...
install: ## Install dependencies and setup environment
	...
	@echo "$(COLOR_GREEN)Installing security tools...$(COLOR_RESET)"
	@go install golang.org/x/vuln/cmd/govulncheck@latest
	@go install github.com/securego/gosec/v2/cmd/gosec@latest
	@go install honnef.co/go/tools/cmd/staticcheck@latest
	@echo "$(COLOR_GREEN)Installing linting tools...$(COLOR_RESET)"
...
lint: ## Run go vet and golangci-lint
	...
	@if command -v golangci-lint >/dev/null 2>&1; then \
		...
		echo "$(COLOR_YELLOW)golangci-lint not installed. Run: make install$(COLOR_RESET)"; \
	fi

##@ Security

security: security-govulncheck security-gosec security-staticcheck ## Run all security scanners
...
security-govulncheck: ## Check for known vulnerabilities in dependencies
	...
	@if command -v govulncheck >/dev/null 2>&1; then \
		...
		echo "$(COLOR_YELLOW)govulncheck not installed. Run: make install$(COLOR_RESET)"; \
	fi

security-gosec: ## Run Go security checker (SAST)
	@echo "$(COLOR_GREEN)Running gosec...$(COLOR_RESET)"
	@if command -v gosec >/dev/null 2>&1; then \
		gosec -fmt text -confidence medium -severity medium ./...; \
	else \
		echo "$(COLOR_YELLOW)gosec not installed. Run: make install$(COLOR_RESET)"; \
	fi

security-staticcheck: ## Run staticcheck for bugs and code issues
	@echo "$(COLOR_GREEN)Running staticcheck...$(COLOR_RESET)"
	@if command -v staticcheck >/dev/null 2>&1; then \
		staticcheck ./...; \
	else \
		echo "$(COLOR_YELLOW)staticcheck not installed. Run: make install$(COLOR_RESET)"; \
	fi
